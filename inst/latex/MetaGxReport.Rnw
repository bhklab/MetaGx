...
geneInfoLists = getGenesProgValue(geneSigList, geneDirecList, cancerType, subtype, survivalMetric, numGroups, dataNames, patientNames, patientScoresList, soloGeneAnalysis, removeMid, censorTime, addBenchmarks, genesRequired)
...
if(grepl("metastasis", survivalMetric, ignore.case = TRUE))
{
   dataStr = "Distant metastasis free survival"
   survivalMetric = "metastasis"
}
if(grepl("hierarchy", survivalMetric, ignore.case = TRUE))
{
   dataStr = "Hierarchy survival (1. relapse free survival, 2. distant metastasis free survival, 3. overall survival)"
   survivalMetric = "hierarchy"
}
...
if(is.null(patientNames) == FALSE)
  cat("\\item{Only patients names provided were used, note that some were likely removed due to missing survival information}")
if(length(patientScoresList) == 0){
  ...
  genesRequired = -1
    cat("\\item{Risk predictions were not calculated as patient scores were provided}")
}
...
cat("\\section{Gene Signature Info}")

for(i in 1:length(geneSigList))
{
  ################################Put tables describing the signature in the report###################################
  cat("\\subsection{", names(survFramesList)[i],"}")
  cat("\\subsubsection{Genes in the Signature}")
  geneFrame = genesFrameList[[i]]
  #longInds = nchar(geneFrame$Description) > 45
  #geneFrame$Description = substr(geneFrame$Description, 1, 45)
  #geneFrame$Description[longInds] = paste0(geneFrame$Description[longInds], "...")
  ...
  geneSigTab = xtable::xtable(geneSigTab, digits = 5, caption = tableCap, align=c("p{2.5cm\\textwidth}","p{2.5cm\\textwidth}","p{8cm\\textwidth}","p{2.5cm\\textwidth}","p{3cm\\textwidth}"))
  addtorow = list()
  addtorow$pos = list()
  addtorow$pos[[1]] = c(0)
  ...
  cat("\\subsubsection{Genes Presence/Absence in Datasets}")
...
if(numSigs > 1)
{
  cat("\\section{Signature Correlation Heatmaps}")
  
  for(i in 1:length(geneInfoLists$sigCorrelationList))
  {
    if(i == 1){
      cat("\\subsection{Signature Correlations using All Patients}") 
    }else{
      cat("\\subsection{Signature Correlations using", names(geneInfoLists$sigCorrelationList)[i],"Patients}")
    }
    an = paste(i, "heatmap.pdf", sep = "")
    aname = paste(system.file("latex", package = "metaGx"),"/",i,"heatmap", sep = "")
    pdf(file = an, width = 14, height = 15, onefile = FALSE)
  
    corMat = geneInfoLists$sigCorrelationList[[i]]
    ...
    heatmap.2(corMat,  Rowv=FALSE, Colv=FALSE, col = cm.colors(256), scale="column", margins=c(12,12), cellnote = corMat, trace="none", notecex = 1.7, notecol = "black", key = FALSE, main = "Pearson Correlation Between Signatures", colsep = c(1:numSigs), rowsep = c(1:numSigs), sepcolor = "black", sepwidth = c(.01, .01), colRow = colorVec, colCol = colorVec, dendrogram = "none",cexRow = 1.6, cexCol = 1.6)      
  
    cat(paste("\\includegraphics[bb = 100 158 433 1112, scale = .55]{{", aname, "}.pdf}", sep = ""))
    dev.off()
    cat("\\clearpage")
...
cat("\\section{Signature Risk Prediction Boxplots}")
  
for(i in 1:numSigs)
{
  cat("\\subsection{", names(survFramesList)[i],"}")
  
    an = paste(i, "boxplot.pdf", sep = "")
    aname = paste(system.file("latex", package = "metaGx"),"/",i,"boxplot", sep = "")
    pdf(file = an, width = 14, height = 15, onefile = FALSE)
  
    makeRiskBoxplots(survFramesList[[i]])  
  
    cat(paste("\\includegraphics[bb = 110 158 433 1112, scale = .55]{{", aname, "}.pdf}", sep = ""))
    dev.off()
    cat("\\clearpage")
...
for(i in 1:numSigs)
{
  ...
  for(k in 1:length(sigSurvFrameList))
  {
    ...
    an = paste("sigAllPlotIns",i*length(sigSurvFrameList) + k ,".pdf", sep = "")
    aname = paste(system.file("latex", package = "metaGx"),"/","sigAllPlotIns", i*length(sigSurvFrameList) + k, sep = "")
    #aname = paste("sigAllPlotIns",k, sep = "")
    pdf(an)
    titleStr = paste("Survival Curve \nUsing", names(sigSurvFrameList)[k])

    makeSurvivalPlot(sigSurvFrameList[[k]], as.numeric(numGroups), normalizeEsetScores = FALSE, titleStr)
    dev.off()
    ...
    if(length(unique(sigSurvFrameList[[k]]$dataName)) > 1)
    {
      ...
      an = paste("sigForPlotIns", i*length(sigSurvFrameList) + k, ".pdf", sep = "")
      #aname = paste("sigForPlotIns", k, sep = "")
      aname = paste(system.file("latex", package = "metaGx"),"/","sigForPlotIns", i*length(sigSurvFrameList) + k, sep = "")
      ...
      minPatients = 20
      makeForestPlot(sigSurvFrameList[[k]], minPatients = minPatients, reportSize = TRUE)
...
if(soloGeneAnalysis == TRUE)
{
  ...
  for(i in 1:length(survFramesList))
  {
    ...
    if(length(geneSurvList$`All Patients`) > 0){
      ...
      an = paste("allPlotIns",i*max(lengths(survFramesList))+1, ".pdf", sep = "")
      aname = paste(system.file("latex", package = "metaGx"),"/","allPlotIns",i*max(lengths(survFramesList))+1, sep = "")
      #aname = paste("allPlotIns",i, sep = "")
      pdf(an)
      xLoc = 2
      titleStr = paste(names(geneSurvList)[1])
      makeSurvivalPlot(geneSurvList$`All Patients`, as.numeric(numGroups), normalizeEsetScores = FALSE, titleStr, xLoc = xLoc)
      xLoc = 3.5
      ...
      for(j in 2:(length(geneSurvList) + 1))
      {
        ...
        if(j != (length(geneSurvList) + 1))
        {
          ...
          makeSurvivalPlot(geneSurvList[[j]], as.numeric(numGroups), normalizeEsetScores = FALSE, titleStr, xLoc = xLoc) 
        }
      }
    }else{
