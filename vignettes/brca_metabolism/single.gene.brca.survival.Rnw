...
.plot.survival.curves.with.basal <- function(pooled.eset, survival.type=c("rfs", "os")) {
  ...
  if(survival.type=="rfs") {
    survival.df <- get.survival.data.for.plotting(pooled.eset.with.gene, entrez.ids=entrez.ids[{{current.index}}], num.quantiles=2, survival.type="tumor.recurrence", time.unit="years", additional.colnames.to.keep = c("subtype"))
  } else {
  ...
  if(strata.term==TRUE && length(unique(my.df$datasets)) > 1) {
  ...
  km.coxph.plot(surv.obj ~ groups, my.df, x.label="Time (years)", y.label = ylab, main.title=main.title, show.n.risk = TRUE, n.risk.step=2, leg.text = levels(my.df$groups), leg.pos="topright", leg.inset=0, leg.bty="n", n.risk.cex=0.85, cex=0.4, o.text=text)
  ...
  km.coxph.plot(surv.obj ~ groups, basal.df, x.label="Time (years)", y.label = ylab, main.title=basal.main.title, show.n.risk = TRUE, n.risk.step=2, leg.text = levels(my.df$groups), leg.pos="topright", leg.inset=0, leg.bty="n", n.risk.cex=0.85, cex=0.4, o.text=text)
...
untreated.pvals <- .plot.survival.curves.with.basal(pooled.eset.rfs.untreated, survival.type="rfs")
@
...
all.pvals <- .plot.survival.curves.with.basal(pooled.eset.rfs, survival.type="rfs")
@
\begin{tabularx}{\textwidth}{p{\dimexpr.5\textwidth}p{\dimexpr.5\textwidth}}
...
\pagebreak
\noindent METABRIC - overall survival
