...
if(survival.type == "os") {
  ...
  esets <- lapply(esets, function(eset) eset[,!is.na(eset$recurrence_status) | !is.na(eset$dmfs_status)])
}
...
if(survival.type == "os") {
  surv.type.text <- "overall survival."
} else {
   surv.type.text <- "recurence-free survival. When recurrence-free survival was unavailable, we used distant metastatis-free survival. "
}
@
...
From MetaGxBreast, we selected patients from studies which tracked \Sexpr{surv.type.text} We identified \Sexpr{sum(sapply(esets, function(x) ncol(exprs(x))))} patients from \Sexpr{length(esets)} datasets.
