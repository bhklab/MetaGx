...
\Sexpr{length(unique(pooled.eset.complete.survival$data.source))} independent ovarian cancer gene expression datasets were identified from MetaGxOvarian. Of this meta-dataset, we identified \Sexpr{num.late.hgs} patients with late-stage, high-grade serous ovarian cancer, of which survival data was available for \Sexpr{num.late.hgs.with.survival} patients:
...
<<gene5, child='immune.response.plots.5.Rnw'>>=
@
\end{document}