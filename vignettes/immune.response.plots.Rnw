...
for(gene.name in c("CTLA4", "CD274", "PDCD1")) {
  ...
  tertiles <- cut(expression.values, breaks=quantile(expression.values, probs=c(0,1/3,2/3,1)))
  levels(tertiles) <- c("Low", "Mid", "High")
  pData(pooled.eset) <- cbind(pData(pooled.eset), paste0(gene.name, "tertile"));
}
@
