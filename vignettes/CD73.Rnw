...
\Sexpr{nrow(nt5e.survival.data)} patients with late-stage, high-grade serous ovarian cancer were pooled from MetaGxOvarian, representing \Sexpr{length(unique(nt5e.survival.data$data.source))} datasets:\footnote{This number is slightly higher than the previous analysis, as I have included the 84 patients in GSE51088 (using the genes that are non-NA values in all patients in this dataset).}
...
stat.objects <- lapply(unique(nt5e.survival.data$data.source), function(x) {
  survival.data.subset <- nt5e.survival.data[nt5e.survival.data$data.source==x,]
  c <- survcomp::concordance.index(survival.data.subset$NT5E.expression, survival.data.subset$years_to_event, survival.data.subset$event_status)
  ...
  coxph.summary <- summary(survival::coxph(Surv(survival.data.subset$years_to_event, survival.data.subset$event_status) ~ survival.data.subset$NT5E.quantile))
  p <- coxph.summary$logtest[["pvalue"]]
  return(list(c=c,d=d))
})
names(stat.objects) <- unique(nt5e.survival.data$data.source)
@
...
\vspace*{-100pt}
\begin{figure}[H]
<<d_index_forestplot, out.width="0.8\\textwidth">>=
myspace <- "    "
d.indices <- sapply(stat.objects, function(x) x$d$d.index)
d.se <- sapply(stat.objects, function(x) x$d$se)
d.lower <- sapply(stat.objects, function(x) x$d$lower)
d.upper <- sapply(stat.objects, function(x) x$d$upper)
d.order <- order(d.indices)
d.indices <- d.indices[d.order]
d.se <- d.se[d.order]
d.lower <- d.lower[d.order]
d.upper <- d.upper[d.order]

pooled.d.index.fixed <- combine.est(d.indices, d.se, hetero=FALSE)
pooled.d.index.random <- combine.est(d.indices, d.se, hetero=TRUE)
d.indices <- c(d.indices, meta.fixed=pooled.d.index.fixed$estimate, meta.random=pooled.d.index.random$estimate)
d.se <- c(d.se, meta.fixed=pooled.d.index.fixed$se, meta.random=pooled.d.index.random$se)
d.lower <- c(d.lower, pooled.d.index.fixed$estimate-pooled.d.index.fixed$se*qnorm(0.975), pooled.d.index.random$estimate-pooled.d.index.random$se*qnorm(0.975))
d.upper <- c(d.upper, pooled.d.index.fixed$estimate+pooled.d.index.fixed$se*qnorm(0.975), pooled.d.index.random$estimate+pooled.d.index.random$se*qnorm(0.975))
d.labeltext <- cbind(names(d.indices),c(rep(myspace,length(d.indices))))
survcomp::forestplot.surv(labeltext=d.labeltext, mean=d.indices, lower=d.lower, upper=d.upper, is.summary = c(rep(FALSE, length(d.indices)-2), TRUE, TRUE), zero=1, xlab="d-index")
@
\end{figure}
\vspace*{-100pt}
Concordance indices:\footnote{The function \texttt{survcomp::concordance.index} returns NA values for standard error, so I was unable to compute pooled estimates. Is this a bug in \texttt{survcomp}?\\
Example: \texttt{survcomp::concordance.index(x=1:7,surv.time=c(4,3,1,1,2,2,3), surv.event=c(1,1,1,0,1,1,0))}}
\vspace*{-100pt}
\begin{figure}[H]
<<c_index_forestplot, out.width="0.8\\textwidth">>=
