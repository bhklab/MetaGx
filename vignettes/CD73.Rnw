...
\Sexpr{nrow(pData(pooled.eset))} patients with late-stage, high-grade serous ovarian cancer were pooled from MetaGxOvarian, representing \Sexpr{length(levels(pData(pooled.eset)$data.source))} datasets:
