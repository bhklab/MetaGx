...
Since these results were not so good, we contacted the authors to acquire the original gene list used for clustering. Out of 1500 genes, there was an overlap of \Sexpr{length(intersect(rownames(tcga.clustering.authordata), mad.1500.gene.list))}. 
