...
.getNMFClasses <- function(eset, filter.genes=TRUE, num.genes=2000, rank=4, nrun=100) {
  ...
  if(filter.genes) {
    ...
    expression.matrix <- expression.matrix[mad.vals >= tail(sort(mad.vals),num.genes)[1],]
  }
...
tothill.clustering.probesets <- scan("../../inst/extdata/tothill.clustering.probesets.txt", what=character(0))
## This file was generated on July 31, 2015 with the following code:
#original.tothill.eset <- getGEO("GSE9891", GSEMatrix=TRUE)[[1]]
#tothill.clustering.probesets <- as.character(fData(original.tothill.eset)$ID[apply(exprs(original.tothill.eset), 1, function(x) any(x > 7) & var(x) >= 0.5)])
#write(tothill.clustering.probesets, file="../../inst/extdata/tothill.clustering.probesets.txt", ncolumns = 1)
