...
load("konecny.unique.entrez.ids.RData")
...
#konecny.unique.entrez.ids <- fData(konecny.original.eset)$GENE[ fData(konecny.original.eset)$ID %in% out$V2 ]
...
.getContingencyHeatmap(table(konecny.merged.authorlist$MAYO.C4, konecny.merged.authorlist$nmf.classes)[c(4,3,2,1),c(3,1,4,2)])
@
...
kable(table(tcga.merged.mad$SUBTYPE, tcga.merged.mad$nmf.classes)[c(3,4,1,2),c(2,4,3,1)], format="latex")
@
...
.getContingencyHeatmap(table(tcga.merged.mad$SUBTYPE, tcga.merged.mad$nmf.classes)[c(3,4,1,2),c(2,4,3,1)])
@

The TCGA supplementary states that the ``The final subclasses were defined based on the most stable k-factor decomposition and visual inspection of sample by sample correlation matrices, in both TCGA and Tothill data set.'' Using package \texttt{R} package \texttt{NMF} with \texttt{nrun = 100} (in the original paper, Brunet et al. assert that 20-100 is usually sufficient), the resulting matrix factorization is very unstable.\footnote{By default, \texttt{NMF} uses the factorization with lowest error on the objective function as the representative solution.}

<<tcga_merged_mad_multiple, fig.width=24, fig.height=16, out.width="1\\textwidth", eval=FALSE>>=
...
We contacted the authors to acquire the original gene list used for clustering.\footnote{Out of 1500 genes, there was an overlap of \Sexpr{length(intersect(rownames(tcga.clustering.authordata), mad.1500.gene.list))}. The difference is likely due to the correlation step across platforms, which the original gene list did not account for.}
\vspace*{10pt}
<<TCGA_contingency_authorlist, results="asis">>=
kable(table(tcga.merged.authorlist$SUBTYPE, tcga.merged.authorlist$nmf.classes)[c(3,4,1,2),c(3,4,1,2)], format="latex")
@

<<tcga_authorlist_contingency_heatmap, fig.width=6, fig.height=4, out.width="0.4\\textwidth">>=
.getContingencyHeatmap(table(tcga.merged.authorlist$SUBTYPE, tcga.merged.authorlist$nmf.classes)[c(3,4,1,2),c(3,4,1,2)])
@
...
tothill.cluster.entrez.ids <- tothill.clustering.genes$entrez.id
...
<<tothill_4_contingency_heatmap, fig.width=5, fig.height=4, out.width="0.4\\textwidth">>=
