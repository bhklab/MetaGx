...
<<verhaak_naivebayes, fig.width=10, fig.height=6, out.width="0.7\\textwidth", eval=TRUE>>=
...
tcga.discovery.eset <- esets$TCGA[,match(verhaak.labels.table$ID, esets$TCGA$unique_patient_ID)]
...
verhaak.nb.classes <- lapply(verhaak.validation.esets, function(eset) {
  ...
  current.train.data <- verhaak.train.data[,c(intersecting.genes, "clustering_subtype")]
...
verhaak.dlda.classes <- lapply(verhaak.validation.esets, function(eset) {
  intersecting.genes <- intersect(colnames(verhaak.train.data), rownames(fData(eset)))
  current.train.data <- verhaak.train.data[,c(intersecting.genes, "clustering_subtype")]
  verhaak.dlda.model <- sparsediscrim::dlda(clustering_subtype ~ ., data=verhaak.train.data)
  current.test.data <- t(exprs(eset))
  current.test.data <- as.data.frame(current.test.data)
  current.test.data <- current.test.data[,intersecting.genes]
  predicted.classes <- predict(verhaak.dlda.model, current.test.data)[[1]]
  out.df <- cbind(pData(eset)[,c("days_to_death", "vital_status")], predicted.classes)
  colnames(out.df)[ncol(out.df)] <- "DLDA.subtype"
  out.df$DLDA.subtype <- as.character(out.df$DLDA.subtype)
  return(out.df)
})

for(name in names(verhaak.dlda.classes)) {
  verhaak.dlda.classes[[name]]$data.source <- name
}
verhaak.nearest.centroid.classes <- lapply(verhaak.validation.esets, function(eset) {
  intersecting.genes <- intersect(colnames(verhaak.train.data), rownames(fData(eset)))
  current.train.data <- verhaak.train.data[,c(intersecting.genes, "clustering_subtype")]
  centroids <- lapply(levels(current.train.data$clustering_subtype), function(subtype) {
    return(sapply(current.train.data[current.train.data$clustering_subtype == subtype,colnames(current.train.data) != "clustering_subtype"], mean))
  })
  names(centroids) <- levels(verhaak.train.data$clustering_subtype)
  current.test.data <- t(exprs(eset))
  current.test.data <- current.test.data[,intersecting.genes]
  scc.vals <- sapply(centroids, function(x) apply(current.test.data, 1, function(y) cor(x,y,method='spearman')))
  predicted.classes <- colnames(scc.vals)[apply(scc.vals, 1, which.max)]
  predicted.classes <- factor(predicted.classes, levels=colnames(scc.vals))
  out.df <- cbind(pData(eset)[,c("days_to_death", "vital_status")], predicted.classes)
  colnames(out.df)[ncol(out.df)] <- "Nearest.Centroid.subtype"
  out.df$Nearest.Centroid.subtype <- as.character(out.df$Nearest.Centroid.subtype)
  return(out.df)
})

for(name in names(verhaak.nearest.centroid.classes)) {
  verhaak.nearest.centroid.classes[[name]]$data.source <- name
}
@

<<verhaak_get_merged, eval=TRUE, cache=FALSE, include=TRUE>>=
implemented.ssgsea.scores.and.nb.classes <- data.frame()
...
for(i in 1:length(implemented.verhaak.output)) {
  ...
  implemented.ssgsea.scores.and.nb.classes <- rbind(implemented.ssgsea.scores.and.nb.classes, data.frame(
                                                                      ...
                                                                      nearest.centroid.subtype=verhaak.nearest.centroid.classes[[i]]$Nearest.Centroid.subtype,
                                                                      dlda.subtype=verhaak.dlda.classes[[i]]$DLDA.subtype,
...
From these normalized ssGSEA scores a subtype classification may be as described by Verhaak et al. as follows: first, Immunoreactive and Mesenchymal subtypes are ckassufued by the higher score if corresponding ssGSEA scores exceed a given threshold.\footnote{The authors obtained their thresholds as the lowest ssGSEA scores for samples clustered within Immunoreactive and Mesenchymal subtypes respectively. For our implementation, we used the same TCGA train data with original cluster labels to re-compute these threshold values for each gene list used for classifying new samples.} Overall, this method produces a concordance of \Sexpr{ round(100 * accuracy.classification.with.thresholds, digits=2)}\% of samples classified identically between our implementation and supplementary results.
...
<<verhaak_table_bymax, results='asis', eval=FALSE>>=
...
<<verhaak_table_nearestcentroid, results='asis'>>=
implemented.classes.nearest.centroid <-  verhaak.merged$nearest.centroid.subtype
supplementary.classes.from.threshold <- verhaak.merged$CloverLeaf
addtorow <- list()
addtorow$pos <- list()
addtorow$pos[[1]] <- 0
addtorow$pos[[2]] <- 0
addtorow$command <- c('& Verhaak Subtypes from Supplementary & & &\\\\\n', "Nearest Centroid Verhaak Subtypes & DIF & IMR & MES & PRO \\\\\n")
print(xtable(table(implemented.classes.nearest.centroid, supplementary.classes.from.threshold), caption="Contingency table showing concordance using a Nearest Centroid subtyping classifier and the predictions given in the supplementary of the Verhaak manuscript. The predictions for both implementations were made on the combined >700 sample dataset by taking the max ssGSEA subtype score."), add.to.row=addtorow, include.colnames=FALSE)
@
...
<<verhaak_survival_curves_dlda, fig.height=5, fig.width=5, out.width="0.5\\textwidth", cache=FALSE>>=

km.coxph.plot(surv.obj ~ dlda.subtype, verhaak.merged, x.label="Time (months)", y.label = "Survival", main.title="Survival: DLDA classifier", show.n.risk = FALSE, leg.text = levels(verhaak.merged$dlda.subtype), .col=c("#E38010", "green", "blue", "purple"), leg.pos="topright", leg.inset=0, n.risk.cex=0.85, cex=0.4)
@

<<verhaak_survival_curves_nearest_centroid, fig.height=5, fig.width=5, out.width="0.5\\textwidth", cache=FALSE>>=
km.coxph.plot(surv.obj ~ nearest.centroid.subtype, verhaak.merged, x.label="Time (months)", y.label = "Survival", main.title="Survival: Nearest Centroid classifier", show.n.risk = FALSE, leg.text = levels(verhaak.merged$nearest.centroid.subtype), .col=c("#E38010", "green", "blue", "purple"), leg.pos="topright", leg.inset=0, n.risk.cex=0.85, cex=0.4)
@
\begin{figure}
...
<<verhaak_classifiers, eval=FALSE>>=
...
<<verhaak_boxplot_plotting, out.width="0.6\\textwidth", eval=FALSE>>=
