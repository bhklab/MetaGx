...
} else if(normalization == "z.pergene") {
  esets <- lapply(esets, function(eset) {
    exprs(eset) <- t(scale(t(exprs(eset))))
    return(eset)
  })
} else if(normalization == "z.perpatient") {
  esets <- lapply(esets, function(eset) {
    exprs(eset) <- scale(exprs(eset))
    return(eset)
  })
} else if(normalization == "z.global") {
  esets <- lapply(esets, function(eset) {
    exprs(eset) <- ( exprs(eset) - mean(unlist(exprs(eset))) ) / sd(unlist(exprs(eset)))
    return(eset)
  })
}
@
