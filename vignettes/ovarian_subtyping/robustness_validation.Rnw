...
dataset.names <- c('GSE14764', 'GSE17260', 'GSE26193', 'GSE18520', 'PMID17290060', 'GSE2109', 'GSE51088', 'GSE20565', 'GSE13876', 'E.MTAB.386', 'GSE32062', 'GSE49997', 'GSE9891', 'GSE26712', 'TCGA')
...
for(dirname in c("konecny_nmf_4", "tcga_nmf_4", "tothill_kmeans_4")) {
  ...
  for(current.eset.name in dataset.names) {
    ...
    filenames <- Sys.glob(paste0("aug8clusters/", dirname, "/", current.eset.name, "_*_classes.txt"))
...
<<TCGA_pred_str>>=
tcga.ps.vals <- lapply(1:length(classifier.subtypes$Verhaak), function(x) sapply(1:100, function(y) ps.cluster(classifier.subtypes$Verhaak[[x]], clustering.subtypes$tcga_nmf_4[[x]][[y]])$ps))
...
konecny.ordered.classes <- c("C1_immL", "C2_diffL", "C3_profL", "C4_mesL")
