...
for(dirname in c("konecny_nmf_4", "tcga_nmf_4", "tothill_kmeans_4")) {
  ...
  for(current.eset.name in dataset.names) {
    ...
    filenames <- Sys.glob(paste0("aug11clusters/", dirname, "/", current.eset.name, "_*_classes.txt"))
...
# returns a list of matrices. List elements are datasets, matrix elements are the stat values, by subtype.
#tcga.vi <- lapply(1:length(classifier.subtypes$Verhaak), function(x) {
#  vi.per.subtype <- sapply(tcga.ordered.classes, function(subtype) {
#    sapply(1:100, function(y) mcclust::vi.dist(as.character(classifier.subtypes$Verhaak[[x]] == subtype), clustering.subtypes$tcga_nmf_4[[x]][[y]]))
#  })
#  return(vi.per.subtype)
#})
#names(tcga.vi) <- names(classifier.subtypes$Verhaak)
#tcga.vi.m <- melt(tcga.vi)
#tcga.vi.means <- melt(sapply(tcga.vi, function(x) apply(x, 2, mean)))
#boxplot(value ~ Var1, tcga.vi.means, outline=FALSE, ylab="Variation of Information", main="Verhaak subtypes")
#stripchart(value ~ Var1, tcga.vi.means, add=TRUE, vertical=TRUE, method='jitter', pch=21)
#ggplot(tcga.adj.rand.m, aes(x=L1, y=value, col=Var2)) + geom_jitter(position=position_jitter(0.2)) + scale_y_continuous(limits=c(0,1))
#ggplot(tcga.mean.things, aes(x=Var1, y=value, fill=Var2)) + geom_bar(stat="identity", position="dodge")
@

<<TCGA_pred_str_plot, width=800, height=800, out.width="0.7\\textwidth">>=
...
.getConsistencyValues <- function(classifier.subtypes.list, clustering.subtypes.list, ordered.classes, distance.function) {
    consistency.list.of.matrices <- lapply(1:length(classifier.subtypes.list), function(x) {
      distance.per.subtype <- sapply(ordered.classes, function(subtype) {
        sapply(1:100, function(y) distance.function(as.character(classifier.subtypes.list[[x]] == subtype), clustering.subtypes.list[[x]][[y]]))
      })
      return(distance.per.subtype)
    })
    
    distance.per.subtype.means <- melt(sapply(consistency.list.of.matrices, function(x) apply(x, 2, mean)))
    boxplot(value ~ Var1, distance.per.subtype.means, outline=FALSE, ylab="Stat", main="Boxplot")
    stripchart(value ~ Var1, distance.per.subtype.means, add=TRUE, vertical=TRUE, method='jitter', pch=21)
    return(consistency.list.of.matrices)
}



#tothill.vi <- lapply(1:length(classifier.subtypes$Helland), function(x) {
#  vi.per.subtype <- sapply(tothill.ordered.classes, function(subtype) {
#    sapply(1:100, function(y) mcclust::vi.dist(as.character(classifier.subtypes$Helland[[x]] == subtype), clustering.subtypes$tothill_kmeans_4[[x]][[y]]))
#  })
#  return(vi.per.subtype)
#})
#names(tothill.vi) <- names(classifier.subtypes$Helland)
#tothill.vi.m <- melt(tothill.vi)
#tothill.vi.means <- melt(sapply(tothill.vi, function(x) apply(x, 2, mean)))
#boxplot(value ~ Var1, tothill.vi.means, outline=FALSE, ylab="Variation of Information", main="Helland subtypes")
#stripchart(value ~ Var1, tothill.vi.means, add=TRUE, vertical=TRUE, method='jitter', pch=21)
@

<<Tothill_pred_str_plot, width=800, height=800, out.width="0.7\\textwidth">>=
par(mar=c(10,4.1,4.1,2.1))
...
# returns a list of matrices. List elements are datasets, matrix elements are the stat values, by subtype.
#konecny.vi <- lapply(1:length(classifier.subtypes$Konecny), function(x) {
#  vi.per.subtype <- sapply(konecny.ordered.classes, function(subtype) {
#    sapply(1:100, function(y) mcclust::vi.dist(as.character(classifier.subtypes$Konecny[[x]] == subtype), clustering.subtypes$konecny_nmf_4[[x]][[y]]))
#  })
#  return(vi.per.subtype)
#})
#names(konecny.vi) <- names(classifier.subtypes$Helland)
#konecny.vi.m <- melt(konecny.vi)
#konecny.vi.means <- melt(sapply(konecny.vi, function(x) apply(x, 2, mean)))
#boxplot(value ~ Var1, konecny.vi.means, outline=FALSE, ylab="Variation of Information", main="Konecny subtypes")
#stripchart(value ~ Var1, konecny.vi.means, add=TRUE, vertical=TRUE, method='jitter', pch=21)
@

<<Konecny_pred_str_plot, width=800, height=800, out.width="0.7\\textwidth">>=
par(mar=c(10,4.1,4.1,2.1))
...
barplot(sapply(esets.not.rescaled.classified[dataset.names], function(x) ncol(exprs(x))), horiz=TRUE, las=2, col="blue", main="HGS sample sizes")
par(mar=c(5.1, 4.1, 4.1, 2.1))
@
