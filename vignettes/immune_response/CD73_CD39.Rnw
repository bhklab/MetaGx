...
nt5e.survival.data <- .get.survival.data.for.plotting(pooled.eset, gene.name="NT5E", num.quantiles=3, survival.type="overall.survival", time.unit="years")
@
...
tcga.nt5e.survival.data <- .get.survival.data.for.plotting(esets$TCGA, gene.name="NT5E", num.quantiles=3, survival.type="overall.survival", time.unit="years")
create.survival.plot(
...
%  nt5e.subtype.survival.data <- .get.survival.data.for.plotting(pooled.eset, gene.name="NT5E", num.quantiles=3, time.unit="years", include.sample = pooled.eset$Helland.subtypes == x)
%  create.survival.plot(
...
#subtype.survival.data.list <- lapply(esets, function(x) .get.survival.data.for.plotting(x, gene.name="NT5E", num.quantiles = 3, time.unit = "years", include.sample=x$Helland.subtypes==levels(esets$TCGA$Helland.subtypes)[[1]]))
...
%  nt5e.subtype.survival.data <- .get.survival.data.for.plotting(pooled.eset, gene.name="NT5E", num.quantiles=3, time.unit="years", include.sample = pooled.eset$Verhaak.subtypes == x)
%  create.survival.plot(
%    surv.time=nt5e.subtype.survival.data$years_to_event,
%    surv.event=nt5e.subtype.survival.data$event_status,
%    groups=nt5e.subtype.survival.data$NT5E.quantile,
...
%  nt5e.subtype.survival.data <- .get.survival.data.for.plotting(pooled.eset, gene.name="NT5E", num.quantiles=3, time.unit="years", include.sample = pooled.eset$Bentink.Haibe.Kains.subtypes == x)
%  create.survival.plot(
%    surv.time=nt5e.subtype.survival.data$years_to_event,
%    surv.event=nt5e.subtype.survival.data$event_status,
%    groups=nt5e.subtype.survival.data$NT5E.quantile,
...
pair.survival.data.by.eset <- lapply(esets.with.data.annotation, function(x) .get.survival.data.for.plotting(x, gene.name=c("NT5E", "ENTPD1"), num.quantiles=2, survival.type="overall.survival", time.unit="years"))
pair.survival.data.by.eset <- pair.survival.data.by.eset[sapply(pair.survival.data.by.eset, function(x) nrow(x)>0)]
pair.survival.data.2 <- do.call(rbind, pair.survival.data.by.eset)
pair.survival.data <- .get.survival.data.for.plotting(pooled.eset, gene.name=c("NT5E", "ENTPD1"), num.quantiles=2, survival.type="overall.survival", time.unit="years")
pair.survival.data$Double.high.expression <- factor(pair.survival.data$Double.high.expression, levels=c("FALSE", "TRUE"), ordered=TRUE)
...
@
%\begin{figure}[H]
<<CD73_histogram, out.width="0.37\\textwidth", eval=FALSE>>=
hist(pair.survival.data.2$NT5E.expression, main="CD73 Expression", xlab="Normalized expression")
@
%\end{figure}
%\begin{figure}[H]
<<CD39_histogram, out.width="0.37\\textwidth", eval=FALSE>>=
hist(pair.survival.data.2$ENTPD1.expression, main="CD39 Expression", xlab="Normalized expression")
@
%\end{figure}

<<quantile_combination_plot, fig.width=5, fig.height=5, eval=FALSE>>=
...
pair.survival.data.2$quadrants <- as.factor(as.factor(paste0(paste0("CD73-",pair.survival.data.2$NT5E.quantile), ", ", paste0("CD39-",pair.survival.data.2$ENTPD1.quantile))))
create.survival.plot(
...
create.survival.plot(
  ...
  groups=pair.survival.data.2$quadrants,
...
<<double_high_plot, fig.width=5, fig.height=5, eval=FALSE>>=
...
create.survival.plot(
  ...
  surv.time=pair.survival.data.2$years_to_event,
  surv.event=pair.survival.data.2$event_status,
  groups=pair.survival.data.2$Double.high.expression,
  group.names = c("Upper quantile signature: -", "Upper quantile signature: +"),
  datasets=pair.survival.data.2$data.source,
  xlab="Time (years)",
  time.cens=10,
...
<<nt5e_alone, fig.width=5, fig.height=5, eval=FALSE>>=
create.survival.plot(
  main="Survival: CD73, median-dichotomized",
  surv.time=pair.survival.data$years_to_event,
  surv.event=pair.survival.data$event_status,
  groups=pair.survival.data$NT5E.quantile,
  datasets=pair.survival.data$data.source,
...
<<ENTPD1 alone, fig.width=5, fig.height=5, eval=FALSE>>=
create.survival.plot(
  main="Survival: ENTPD1, median-dichotomized",
  surv.time=pair.survival.data$years_to_event,
  surv.event=pair.survival.data$event_status,
  groups=pair.survival.data$ENTPD1.quantile,
  datasets=pair.survival.data$data.source,
  #group.names=c("CD73 Low", "CD73 High"),
  xlab="Time (years)",
  time.cens=10,
...
create.survival.plot(
  ...
  surv.time=pair.survival.data.2$years_to_event,
  surv.event=pair.survival.data.2$event_status,
  groups=pair.survival.data.2$ENTPD1.quantile,
  datasets=pair.survival.data.2$data.source,
  #group.names=c("CD73 Low", "CD73 High"),
  xlab="Time (years)",
  time.cens=10,
...
create.forest.plot(survival.data = pair.survival.data.by.eset, surv.time.colname="years_to_event", surv.event.colname = "event_status", risk.val.colname = "NT5E.expression", stat="d.index")
@
\end{figure}
Concordance indices:
\begin{figure}[H]
<<c_index_forestplot, fig.width=5, fig.height=4, out.width="0.6\\textwidth">>=
create.forest.plot(survival.data = pair.survival.data.by.eset, surv.time.colname="years_to_event", surv.event.colname = "event_status", risk.val.colname = "NT5E.expression", stat="concordance.index")
@
\end{figure}
...
create.survival.plot(
  main="Survival Plot: pooled samples",
  surv.time=survival.data.tertiles$years_to_event,
  surv.event=survival.data.tertiles$event_status,
  groups=survival.data.tertiles$NT5E.quantile,
  datasets=survival.data.tertiles$data.source,
  xlab="Time (years)",
  reverse.legend.order = TRUE,
  #group.names=paste("CD73", gsub(".*=", "",levels(survival.data.tertiles$NT5E.quantile))),
  #risk.vals=survival.data.tertiles$NT5E.expression,
  stats.to.show=c("n","p"),
  time.cens=10,
  col=c("black", "blue", "red")
  )
...
<<NT5E_survival_bipartition, eval=TRUE, fig.width=6.75, fig.height=8, out.width="0.8\\textwidth", eval=FALSE>>=
create.survival.plot(
  main="Survival: CD73, median-dichotomized",
  surv.time=pair.survival.data.2$years_to_event,
  surv.event=pair.survival.data.2$event_status,
...
<<stat_comparisons, eval=FALSE, eval=FALSE>>=
...
<<stripplot, fig.width=8, fig.height=6, eval=FALSE>>=
tcga.eset <- esets$TCGA
tcga.nt5e.index <- which(fData(tcga.eset)$gene == "NT5E")
pData(tcga.eset)$NT5E.expression <- t(exprs(tcga.eset)[tcga.nt5e.index,,drop=FALSE])
f.test.pval <- anova(lm(NT5E.expression ~ primary_therapy_outcome_success, data=pData(tcga.eset)))$`Pr(>F)`[1]
set.seed(1000)
stripchart(NT5E.expression ~ primary_therapy_outcome_success, data=pData(tcga.eset), method="jitter", vertical=TRUE, main="TCGA data: response to primary therapy")
@

%F-test p-value: \Sexpr{round(f.test.pval, digits=3)}
